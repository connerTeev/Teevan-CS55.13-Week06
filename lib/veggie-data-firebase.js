...
export async function getSortedVeggiesList() {
  const snapshot = await getDocs(collection(db, 'veggies'));
...
export async function getVeggieIds() {
  const snapshot = await getDocs(collection(db, 'veggies'));
  const jsonObj = snapshot.docs.map((document) => ({
    id: document.id,
  }));
...
export async function getVeggieData(idRequested) {
  const docRef = doc(db, 'veggies', idRequested);
